...
def get_not_downloaded_papers(conn, cur, order_type, processing_limit):
    global schema
    cur.execute(
        '''SELECT paper_id, download_url FROM ''' + schema + '''.paper where stage=-1 order by submission_date ''' + order_type + '''  limit %s;''',
        (processing_limit,))
    return cur.fetchall()
...
def get_not_section_extracted_papers(conn, cur, limit):
    global schema
    cur.execute(
        '''select p.paper_id, pt.text from ''' + schema + '''.paper p join ''' + schema + '''.paper_text pt on p.paper_id = pt.paper_id  where p.stage = 1 and p.abstract_added=true and (p.section_extracted is null or p.section_extracted=false) limit %s''',
        (limit,))
    return cur.fetchall()


def get_not_section_extracted_paper_ids(conn, cur, limit):
    global schema
    cur.execute(
        '''select p.paper_id from ''' + schema + '''.paper p where p.stage = 1 and p.abstract_added=true and (p.section_extracted is null or p.section_extracted=false) limit %s''',
        (limit,))
    return cur.fetchall()
...
def get_section_extracted_paper_ids(conn, cur, dataset_version, order_type, limit):
    global schema
    cur.execute(
        '''select p.paper_id, p.name, p.tags, p.submission_date from ''' + schema + '''.paper p  where p.section_extracted=true and (p.dataset_version <> %s or p.dataset_version is null) order by submission_date ''' + order_type + ''' limit %s''',
...
def get_not_abstract_added_papers(conn, cur, limit):
    global schema
    cur.execute(
        '''SELECT paper_id FROM ''' + schema + '''.paper where abstract_added is null or abstract_added=false limit %s''',
        (limit,))
    return cur.fetchall()
